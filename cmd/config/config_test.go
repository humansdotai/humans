...
func TestSetCoinType(t *testing.T) {
	...
	require.Equal(t, int(humans.Bip44CoinType), int(config.GetCoinType()))
...
func TestHDPath(t *testing.T) {
	params := *hd.NewFundraiserParams(0, humans.Bip44CoinType, 0)
	hdPath := params.String()
	require.Equal(t, "m/44'/60'/0'/0/0", hdPath)
	require.Equal(t, hdPath, humans.BIP44HDPath)
}
