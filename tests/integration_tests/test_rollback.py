...
def update_node_cmd(path, cmd, i):
    ...
    for section in ini.sections():
        if section == f"program:humans_9000-1-node{i}":
            ini[section].update(
                {
...
def custom_humans(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-humansd.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/humansd"
    )
    print(broken_binary)
...
def test_rollback(custom_humans):
    """
    ...
    update_node_cmd(custom_humans.base_dir, "humansd", 1)
    supervisorctl(custom_humans.base_dir / "../tasks.ini", "update")
    wait_for_port(target_port)
