...
**Prerequisite**: in the repo's root, run `make install` to install the `humansd` and `humansd` binaries. When done, come back to this directory.
...
To run the tests, you can use the `test-helper.js` utility to test all suites under `ganache` or `humans` network. The `test-helper.js` will help you spawn an `humansd` process before running the tests.

You can simply run `yarn test --network humans` to run all tests with humans network, or you can run `yarn test --network ganache` to use ganache shipped with truffle. In most cases, there two networks should produce identical test results.

If you only want to run a few test cases, append the name of tests following by the command line. For example, use `yarn test --network humans basic` to run the `basic` test under `humans` network.

If you need to take more control, you can also run `humansd` using:
...
Keep the terminal window open, go into any of the tests and run `yarn test-humans`. You should see `humansd` accepting transactions and producing blocks. You should be able to query for any transaction via:

- `humansd query tx <cosmos-sdk tx>`
...
The [`init-test-node.sh`](./init-test-node.sh) script sets up humans with the following accounts:
...
Each with roughly 100 ETH available (1e18 heart).

Running `humansd keys list --keyring-backend=test` should output:
