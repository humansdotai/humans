...
For example, on Ethereum, the `evm_denom` would be `ETH`. In the case of Humans, the default denomination is the **[atto heart](notion://www.notion.so/docs/basics/heart.md)** (used on the Evmos testnets). In terms of precision, the `PHOTON` and `ETH` share the same value, *i.e* `1 PHOTON = 10^18 atto heart` and `1 ETH = 10^18 wei`.
