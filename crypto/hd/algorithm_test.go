...
import (
	...
	enccodec "github.com/0x4139/humans/encoding/codec"
...
func TestKeyring(t *testing.T) {
	...
	kr, err := keyring.New("humans", keyring.BackendTest, dir, mockIn, TestCodec, EthSecp256k1Option())
	...
	info, mnemonic, err := kr.NewMnemonic("foo", keyring.English, humans.BIP44HDPath, keyring.DefaultBIP39Passphrase, EthSecp256k1)
	require.NoError(t, err)
...
func TestDerivation(t *testing.T) {
	bz, err := EthSecp256k1.Derive()(mnemonic, keyring.DefaultBIP39Passphrase, humans.BIP44HDPath)
	require.NoError(t, err)
	require.NotEmpty(t, bz)
	...
	path := MustParseDerivationPath(humans.BIP44HDPath)
	account, err := wallet.Derive(path, false)
	require.NoError(t, err)
	...
	// Equality of Humans implementation
	...
	// Equality of Eth and Humans implementation
	...
	// Inequality of wrong derivation path of Eth and Humans implementation
